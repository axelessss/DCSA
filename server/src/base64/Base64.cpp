...
char *av__encode(char *out, int out_size, const uint8_t *in, int in_size)
{
    return av_base64_encode_l(out, &out_size, in, in_size);
}

string base64::Encode(const string &txt)
{
    if (txt.empty())
    {
        return "";
...
string base64::Decode(const string &txt)
{
    if (txt.empty())
    {
        return "";
...
vector<string> base64::Split(const string &s, const char *delim)
{
    vector<string> ret;
    size_t last = 0;
